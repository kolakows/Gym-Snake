...
policyConstructor = PolicySimpleConv

render = False
render_every_eps = 5000
...
for episode in range(episodes):
    ...
    while not done:
        ...
        #obs_norm = (obs_norm - obs_norm.mean())/(obs_norm.var() * 8.7)
        obs_norm = np.moveaxis(obs_norm, 2, 0)
    ...
    env.reset()
    snake_len = env.snake_size
    if episode % render_every_eps == render_every_eps-1:
        #time.sleep(10)
        render = True
    else:
        render = False
